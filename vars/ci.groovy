    ...
    node ('workstation') {
        ...
        if (env.JOB_BASE_NAME ==~ "PR.*") {
            ...
            stage('Release'){
               ...
               sh 'curl -sSf -u "admin:${ARTIFACTORY_PASSWORD}" -X PUT -T ${repo_name}-${TAG_NAME}.zip http://artifactory.sbadiger93.online:8081/artifactory/example-repo-local/${repo_name}-${TAG_NAME}.zip'
              }
        }
        else {
            sh 'echo branch'
